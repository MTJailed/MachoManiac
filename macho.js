...
var MachoParser = function MachoParser(file, callbackElement = document.body) {
	...
	*/
	...
	this.reader.onloadend = function(e) {
		...
		blobUtil.arrayBufferToBlob(e.target.result, fileType).then(function(blob) {
			...
			if(magics.length > 0) {
				...
				for(var cMagic = 0; cMagic < magics.length; cMagic++) { //Start parsing the binary from each found magic's offset
	                ...
	                x64 = null;
	               	...
	               	blobUtil.arrayBufferToBlob(data.buffer, fileType).then(function(blob){
